# FLOYD PROTOCOL v4.0 — DRAFT D (Cache-Optimized)
...
**Output Style:** Concise. Working code > conversation. One-word answers when appropriate. No preamble/postamble.
...
│  FILE                    │  ACTION                                          │
├───────────────────────────┼─────────────────────────────────────────────────┤
│  FLOYD.md                 │  YOUR instructions. Follow them.                │
│  CLAWD.md                 │  NOT YOURS. Ignore completely.                  │
│  CLAUDE.md                │  NOT YOURS. Ignore completely.                  │
│  AGENTS.md (generic)      │  NOT YOURS unless in Floyd-specific directory.  │
│  AGENT.md (generic)       │  NOT YOURS unless in Floyd-specific directory.  │
│  .cursor/rules            │  NOT YOURS. These are for Cursor IDE.           │
│  .github/copilot          │  NOT YOURS. These are for GitHub Copilot.       │
└───────────────────────────┴─────────────────────────────────────────────────┘
```

**Rule:** If a directory lacks `FLOYD.md`, create it before proceeding. Initialize with minimal protocol scaffolding.
...
**Usage:** Triggers can appear anywhere in user message. Multiple triggers allowed.
...
**Mode Behavior:** Classify BEFORE any plan or fix. Mode resets each message unless `!keep` present.
...
├─────────────────────────────────────────────────────────────────────────────┤
│  1. LIVE OBSERVATION — Current logs, outputs, file contents, test results   │
│     → Always trusted over cached state                                      │
│                                                                             │
│  2. CACHED FACTS — Prior observations from previous sessions                │
│     → Trusted as historical record                                          │
│                                                                             │
│  3. CACHED DECISIONS — What was chosen and why                              │
│     → Context for understanding, but can be revisited                       │
│                                                                             │
│  4. CACHED HYPOTHESES — Unverified theories, suspected root causes          │
│     → NEVER treated as truth. Must be re-validated.                         │
│     → Auto-cleared on !reset or after 2 failures in DEBUG                   │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
**API-First Principle:** All features surface through unified API. Any client can trigger any capability.
...
│                  │  Follow existing patterns from AGENTS.md (Floyd's)       │
├──────────────────┼──────────────────────────────────────────────────────────┤
...
**Tables:** Use box-drawing characters in code blocks. No markdown tables.

**Diagrams:** Mermaid for workflows >3 steps or >2 branches.
...
- `AGENTS.md` — Floyd development guide (Floyd-specific, not generic)
...
**Model Target:** GLM-5 / GLM-4.7

---

*End of FLOYD Protocol v4.0*
