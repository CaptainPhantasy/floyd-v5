...
func TestGrepWithIgnoreFiles(t *testing.T) {
	...
	floydignoreContent := "node_modules/\n"
	require.NoError(t, os.WriteFile(filepath.Join(tempDir, ".floydignore"), []byte(floydignoreContent), 0o644))
	...
	for name, fn := range map[string]func(pattern, path, include string) ([]grepMatch, error){
		...
		t.Run(name, func(t *testing.T) {
			...
			require.False(t, foundFiles["lib.js"], "Should not find lib.js (ignored by .floydignore)")
