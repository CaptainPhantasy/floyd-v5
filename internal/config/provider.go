	...
	providerOnce.Do(func() {
		...
		wg.Go(func() {
			...
			if err != nil {
				...
				errs = append(errs, fmt.Errorf("Floyd was unable to fetch an updated list of providers from %s. Consider setting FLOYD_DISABLE_PROVIDER_AUTO_UPDATE=1 to use the embedded providers bundled at the time of this Floyd release. You can also update providers manually. For more info see floyd update-providers --help.\n\nCause: %w", catwalkURL, providerErr)) //nolint:staticcheck
				return
			}
		...
		wg.Go(func() {
			...
			if err != nil {
				errs = append(errs, fmt.Errorf("Floyd was unable to fetch updated information from Hyper: %w", err)) //nolint:staticcheck
				return
			}
