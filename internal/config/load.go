...
import (
	...
	"github.com/legacy-ai/floyd/internal/log"
...
func PushPopFloydEnv() func() {
	found := []string{}
	for _, ev := range os.Environ() {
		if strings.HasPrefix(ev, "FLOYD_") {
			pair := strings.SplitN(ev, "=", 2)
			if len(pair) != 2 {
				continue
			}
			found = append(found, strings.TrimPrefix(pair[0], "FLOYD_"))
		}
	}
	...
	for _, ev := range found {
		os.Setenv(ev, os.Getenv("FLOYD_"+ev))
	}
...
func (c *Config) setDefaults(workingDir, dataDir string) {
	...
	// Apply defaults to MCP configurations.
	c.applyMCPDefaults()
...
// applyMCPDefaults populates the default MCP server configurations. User
// config entries take precedence — defaults are only applied for servers
...
func (c *Config) applyMCPDefaults() {
	defaultMCPs := map[string]MCPConfig{
		...
		"floyd-devtools-server": {
			Type:    MCPStdio,
			Command: "node",
			Args:    []string{"/Volumes/Storage/MCP/floyd-devtools-server/dist/index.js"},
		},
		"floyd-safe-ops-server": {
			Type:    MCPStdio,
			Command: "node",
			Args:    []string{"/Volumes/Storage/MCP/floyd-safe-ops-server/dist/index.js"},
		},
		...
		"novel-concepts-server": {
			Type:     MCPStdio,
			Command:  "node",
			Args:     []string{"/Volumes/Storage/MCP/novel-concepts-server/dist/index.js"},
			Disabled: true,
		},
		"omega-v2": {
			Type:     MCPStdio,
	...
	for name, defaultCfg := range defaultMCPs {
		if _, exists := c.MCP[name]; !exists {
			c.MCP[name] = defaultCfg
		}
	}
}
...
func GlobalConfig() string {
	if floydGlobal := os.Getenv("FLOYD_GLOBAL_CONFIG"); floydGlobal != "" {
		return filepath.Join(floydGlobal, fmt.Sprintf("%s.json", appName))
	}
...
func GlobalConfigData() string {
	if floydData := os.Getenv("FLOYD_GLOBAL_DATA"); floydData != "" {
		return filepath.Join(floydData, fmt.Sprintf("%s.json", appName))
	}
	...
	// for windows, it should be in `%LOCALAPPDATA%/floyd/`
	// for linux and macOS, it should be in `$HOME/.local/share/floyd/`
...
func GlobalSkillsDirs() []string {
	if floydSkills := os.Getenv("FLOYD_SKILLS_DIR"); floydSkills != "" {
		return []string{floydSkills}
	}
...
// GlobalAgentsDirs returns the default directories for Agent definitions.
func GlobalAgentsDirs() []string {
	if floydAgents := os.Getenv("FLOYD_AGENTS_DIR"); floydAgents != "" {
		return []string{floydAgents}
	}
	...
	// Get the floyd binary's directory for extensibility agents
	var floydDir string
	if execPath, err := os.Executable(); err == nil {
		floydDir = filepath.Dir(execPath)
	}

	dirs := []string{
		filepath.Join(configBase, appName, "agents"),
	}

	// Add extensibility agents directory if floyd binary location is found
	if floydDir != "" {
		extensibilityAgents := filepath.Join(floydDir, "extensibility", "agents")
		if _, err := os.Stat(extensibilityAgents); err == nil {
			dirs = append(dirs, extensibilityAgents)
		}
	}
