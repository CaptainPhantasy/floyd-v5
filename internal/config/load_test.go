...
func TestConfig_setDefaults(t *testing.T) {
	...
	require.Equal(t, filepath.Join("/tmp", ".floyd"), cfg.Options.DataDirectory)
	require.Equal(t, "FLOYD.md", cfg.Options.InitializeAs)
...
func TestConfig_setupAgentsWithDisabledTools(t *testing.T) {
	...
	expectedCoder := resolveAllowedTools(allToolNames(), cfg.Options.DisabledTools)
	assert.Equal(t, expectedCoder, coderAgent.AllowedTools)

	taskAgent, ok := cfg.Agents[AgentTask]
	require.True(t, ok)
	assert.Equal(t, resolveReadOnlyTools(expectedCoder), taskAgent.AllowedTools)
}
...
func TestConfig_setDefaultsDisableDefaultProvidersEnvVar(t *testing.T) {
	t.Run("sets option from environment variable", func(t *testing.T) {
		t.Setenv("FLOYD_DISABLE_DEFAULT_PROVIDERS", "true")
