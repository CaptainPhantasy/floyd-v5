...
func TestFloydIgnore(t *testing.T) {
	tempDir := t.TempDir()
	t.Chdir(tempDir)
	...
	// Create a .floydignore file that ignores .log files
	require.NoError(t, os.WriteFile(".floydignore", []byte("*.log\n"), 0o644))
...
func TestShouldExcludeFile(t *testing.T) {
	...
	// Create .floydignore file
	floydignoreContent := "custom_ignored/\n"
	if err := os.WriteFile(filepath.Join(tempDir, ".floydignore"), []byte(floydignoreContent), 0o644); err != nil {
		t.Fatalf("Failed to create .floydignore: %v", err)
	}
	...
	require.True(t, ShouldExcludeFile(tempDir, customIgnored), "Expected custom_ignored to be ignored by .floydignore")
...
func TestShouldExcludeFileHierarchical(t *testing.T) {
	...
	// Create .floydignore in subdir that ignores normal_nested
	subFloydignore := "normal_nested/\n"
	if err := os.WriteFile(filepath.Join(subDir, ".floydignore"), []byte(subFloydignore), 0o644); err != nil {
		t.Fatalf("Failed to create subdir .floydignore: %v", err)
	}

	// Test hierarchical ignore behavior - this should work because the .floydignore is in the parent directory
	require.True(t, ShouldExcludeFile(tempDir, nestedNormal), "Expected normal_nested to be ignored by subdir .floydignore")
