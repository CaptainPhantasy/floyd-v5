...
func (m *Chat) RemoveMessage(id string) {
	...
	// Clean up nested tool IDs before removal.
	if item, ok := m.list.ItemAt(idx).(chat.NestedToolContainer); ok {
		for _, nested := range item.NestedTools() {
	...
	for i := idx; i < m.list.Len(); i++ {
		if item, ok := m.list.ItemAt(i).(chat.MessageItem); ok {
			...
			// Also re-register nested tool IDs.
