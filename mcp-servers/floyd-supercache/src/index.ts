...
// Cache storage paths
// Support --data-dir CLI argument for per-project isolation
// Falls back to global ~/.floyd/supercache for backwards compatibility
const CACHE_DIR = (() => {
  const idx = process.argv.indexOf('--data-dir');
  if (idx !== -1 && process.argv[idx + 1]) {
    return process.argv[idx + 1];
  }
  return join(homedir(), '.floyd', 'supercache');
})();
